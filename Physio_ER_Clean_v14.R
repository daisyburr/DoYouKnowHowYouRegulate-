...
summ(glmer_ERQ_STAI, center = TRUE, confint = TRUE) #exp = TRUE for ORs
...
glmer_LL_STAI <- interact_plot(glmer_ERQ_STAI, pred = z_STIM_LL, modx = STAI, interval = TRUE, outcome.scale = "response", x.label = "Levator (z)", y.label = "Predicted Rrobability \n of Reappraising", data = glmer, legend.main = "Trait Anxiety") + theme(axis.text.x = element_text(angle = 90, hjust = 1))  + theme_apa()
...
library(tidyr)
#predict distance to uninstructed from instructed condition ERQ, and STAI
...
dist_3_effect <- effect_plot(dist_3, pred = STAI, interval = TRUE, outcome.scale = "response", x.label = "STAI", legend.main = "Instructed Condition", y.label = "Dissimilarity to \n Instructed", data = data_long) 
...
dist_3_int <- interact_plot(dist_3, pred = STAI, modx = ins_cond, interval = TRUE, outcome.scale = "response", x.label = "STAI", legend.main = "Instructed Condition", y.label = "Dissimilarity to \n Instructed", data = data_long) 
